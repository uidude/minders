...
// Sure, why not!
/*
- `create(Partial<Item>) => item`
- `delete(id)`
- `nest(id)`
- `unnest(id)`
- `expandAll()`
- `collapseAll()`
- `createItemAfter(item, Partial<Item>) => item`
- `move(item, newParent)`
- `bump(item)`
- `touch(item)`
*/
/*
TODO:
- Mark outline item fields as deprecated

*/
/*
Question: Will these always in future require 
ideal usage
...
*/
/*
caching datastore API
// TODO: The cache needs to be configured outside of any specific method
// This configuration can also possibly handle invalidation logic?
const [minders, cache] = useCachingDataStore(Minders, memoryCache(...));

// To listen
cache.listen(id, fn);
cache.listen([id1, id2], fn);

// To invalidate
cache.invalidate(id);
cache.invalidateAll();
cache.invalidateWhere(fn);

// To get latest
const latest = useLatest(minders, id);
maybe
const [current, setCurrent] = useLiveGet(Minders, id);
although you almonst never want the page just randomly updating. So maybe you always listen?
*/
...
export type MinderScreenContextType = {
  ...
  trackSelection(minderId: Opt<string>, range?: SelectionRange): void;

  /** Request that the app select new content in the UI */
  requestSelect(minderId: string, selector?: Selector): void;
...
export function MinderScreenContextProvider(props: {
  ...
  function isSelected(minderId: string) {
    return trackedSelection.current.minderId === minderId;
  }
  ...
  function trackSelection(minderId: Opt<string>, range?: SelectionRange) {
    trackedSelection.current = {minderId, range};
  }
